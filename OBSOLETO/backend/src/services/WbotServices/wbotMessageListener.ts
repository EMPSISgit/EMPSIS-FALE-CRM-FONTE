...
const verifyMediaMessage = async (
  ...
  const messageData = {
    ...
    body: body ? formatBody(msg.key.fromMe ? `${ticket.user.name}: ${body}` : `${ticket.contact.name}: ${body}`, ticket.contact) : media.filename,
...
export const verifyMessage = async (
  ...
  const rawBody = msg.key.fromMe ? `*${ticket.user.name}*: \n${body}` : `*${ticket.contact.name}:*\n ${body}`;
