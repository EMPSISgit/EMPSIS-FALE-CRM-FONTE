...
export const forwardMessage = async (req, res) => {
    try {
      ...
      console.log('req: ##############################')
      console.log(req)
      // Chama um serviço que “copia” a mensagem para outro contato
      const newMessage = await ForwardMessageService({ messageId, contactId });
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  const { ticketId } = req.params;
  const { userName, body, quotedMsg }: MessageData = req.body;
  ...
  if (medias) {
    ...
    const send = await SendWhatsAppMessage({ body, ticket, quotedMsg, userName });
  }
