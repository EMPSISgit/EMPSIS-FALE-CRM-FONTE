...
const MessageInputCustom = (props) => {
  ...
  useEffect(() => {
      ...
      const handleDrop = (e) => {
        ...
        if (e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files.length > 0) {
          // Cria um evento sintético para reutilizar a lógica do FileInput
          const syntheticEvent = { target: { files: e.dataTransfer.files } };
          handleChangeMedias(syntheticEvent);
  ...
  const handleDrop = (e) => {
    ...
    if (e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      // Cria um "evento sintético" com a mesma estrutura que o onChange do input file
      const syntheticEvent = { target: { files: e.dataTransfer.files } };
      handleChangeMedias(syntheticEvent);
      e.dataTransfer.clearData();
    }
  ...
  if (medias.length > 0)
    return (
      <Paper elevation={0} square className={classes.viewMediaInputWrapper}>
        ...
        {/* Seus componentes: EmojiOptions, FileInput, CustomInput, ActionButtons, etc. */}
        </div>
